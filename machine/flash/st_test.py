...
def tft_init(disp):
    ...
    disp.tft_writecmddata(_MADCTL, bytearray([0b10110000]))
    disp.tft_writecmddata(_CASET, pack(">HH", 0, 159)
                          )  # set width from 0 to 159
    # set height from  0 to 127
    disp.tft_writecmddata(_PASET, pack(">HH", 0, 127))
...
def _recvTextCallback(webSocket, msg):
    ...
    if type == "R":
        roll = (val - 50) * (pi/8) / 50.0
    if type == "P":
        pitch = (val - 50) * (pi/8) / 50.0
    if type == "U":
        up_down = val//2 + 40
...
calbration = ((0.8, -0.3, -0.6), (-0.7, 0.1, 0.25),
              (-0.8, 0.1, 0.1), (0.7, -0.1, 0))
...
def update():
    for joint in range(3):
        for leg in range(4):
            robot.set_servo(body_position[leg][joint] * body_direction[leg]
                            [joint] + calbration[leg][joint], body_servo[leg][joint])
...
def update_status():
    ...
    if not ap_if.active():
        tft.text(40, 15, 'AP off')
    else:
        tft.text(40, 15, ap_if.ifconfig()[0])
    if not sta_if.isconnected():
        tft.text(40, 30, 'STA off')
    else:
        tft.text(40, 30, sta_if.ifconfig()[0])
    if voltage < 7:
...
def menu(title, menu_list):
    ...
    while True:
        ...
        if old != new and not new:
            ...
            if pos > max - 1:
                pos = 0
            tft.text(25, 80, menu_list[pos] + " " *
                     (max_item - len(menu_list[pos])))
        old = new
        new2 = switch2.value()
        if old2 != new2 and not new2:
            break
        time.sleep(0.1)
...
try:
    ...
    update_status()
    pos = menu("Choose WiFi Mode", [
               "Acess Mode", "Station Mode", "Don't Connect"])
    ...
    if pos == 1:
        ...
        sta_if.ifconfig(('192.168.1.115', '255.255.255.0',
    ...
    # WebSockets without new threads
    ...
    while True:
        body_cartesian = [[50, 35, up_down], [50, 35, up_down], [
            50, -35, up_down], [50, -35, up_down]]
        update_status()
